...
unsafe fn draw_item_hl_text(dc: HDC, s: &AppState, list_index: usize, rect: &RECT, selected: bool) {
    ...
    let rgn = CreateRoundRectRgn(rect.left + 2, rect.top + 2,
        rect.right - 2, rect.bottom - 2, rcr, rcr);
...
unsafe fn draw_filtered_item(hdc: HDC, s: &AppState, list_index: usize, rect: &RECT) {
    ...
    if is_sel {
        ...
        let rgn = CreateRoundRectRgn(rect.left + 2, rect.top + 2,
            rect.right - 2, rect.bottom - 2, rcr, rcr);
        let brush = CreateSolidBrush(colorref(s.accent_color));
...
unsafe extern "system" fn wndproc(
    ...
    match msg {
        ...
        WM_PAINT => {
            ...
            // 输入框
            let input_bg = CreateSolidBrush(colorref(s.input_bg_color));
            let ir = &s.input_rect;
            let ic = (s.round_corner * 3 / 4).max(1);
            let rgn = CreateRoundRectRgn(ir.left, ir.top, ir.right, ir.bottom, ic, ic);
            FillRgn(hdc, rgn, input_bg);
            let _ = DeleteObject(HGDIOBJ(rgn.0));
            let _ = DeleteObject(HGDIOBJ(input_bg.0));
