...
| 命令执行 | `CreateProcessW`（`Win32_System_Threading`） | 支持执行任意命令行（程序+参数） |
...
├── executor.rs  — 执行器：URL → 浏览器打开；.exe / 路径 → `ShellExecuteW` 启动；反引号命令 → `CreateProcessW` 执行
└── tray.rs      — 系统托盘图标与右键菜单
```
...
- 命令不需要 `.exe` 后缀（系统自动从 PATH 中查找）
- 带空格的程序路径用引号包裹，如 `` `"C:\Program Files\App\app.exe" --flag` ``
- 控制台命令会弹终端窗口。要窗口执行完不关，配合 `cmd /k` 使用
- 使用 `CreateProcessW` 执行，系统自己解析命令行（程序名+参数），不手动拆分

### 值类型自动判断

| 特征 | 行为 |
|---|---|
| 以 `` ` `` 开头和结尾 | 命令行执行（`CreateProcessW`） |
...
在 `fonts/` 目录下放入 `.ttf` 或 `.otf` 文件，程序自动注册为私有字体（仅当前进程可见，不安装到系统）。

- **不配 `_font`**：按文件名排序取第一个字体，自动读取其家族名称并生效
- **显式指定**：在 `_font` 中填写字体家族名称即可
- **热重载**：改 `fonts/` 里的文件后按热键或点托盘图标即可生效，无需重启

> **如何查看字体家族名称？** 双击 `.ttf` / `.otf` 文件，弹出窗口最顶部显示的名称就是家族名称。
> 例如 `SmileySans-Oblique.ttf` 的家族名称是 `得意黑`，`LXGWWenKai-Regular.ttf` 的家族名称是 `LXGW WenKai`。
...
| `_font` | 字体家族名称（不是文件名；见 fonts/ 说明） | `Segoe UI` |
| `_font_size` | 字号 | `18` |
